...
func (m *constrainedAgentMiddleware) AfterModelRewriteState(ctx context.Context, state *adk.ChatModelAgentState, mc *adk.ModelContext) (context.Context, *adk.ChatModelAgentState, error) {
	...
	if last != nil {
		...
		if last.ResponseMeta != nil && last.ResponseMeta.Usage != nil {
			tokens = last.ResponseMeta.Usage.TotalTokens
		}
		if tokens <= 0 {
			characters := 0
			...
			// Deliberately conservative fallback: count the complete request
			// history because providers account for prompt tokens on every turn.
