...
func TestIncidentGraphAndCausalToolsShareWorkflowState(t *testing.T) {
	capabilities, err := buildConstrainedDiagnosisCapabilities(constrainedToolDeps{Reasoning: reasoning.New(reasoning.DefaultConfig()), Causal: causal.DefaultMatcher()})
	if err != nil {
		t.Fatal(err)
	}
	...
	budget := safety.NewBudgetController(&domain.AgentBudgetState{}, map[string]domain.AgentBudget{DiagnosisAgentName: {MaxToolUses: 20, MaxTokens: 10000, MaxCorrections: 2}}, map[string]int{})
